...
use crate::function::sync::model::{
    FileSystemState, DiffResult, DiffEntry, DiffType, EntryType
};
    ...
    for (path, local_entry) in &local.entries {
        ...
        if let Some(remote_entry) = remote.entries.get(path) {
            ...
            let is_same = if local_entry.is_directory() && remote_entry.is_directory() {
                ...
                let hash_same = if config.compare_hash {
                    match (&local_entry.content_hash, &remote_entry.content_hash) {
...
/// 打印差异报告（用于调试）
pub fn print_diff_report(diff: &DiffResult) {
    println!("差异报告 (生成于 {})", diff.diff_time);
    println!("----------------------------------------");
    println!("添加: {}", diff.added_count());
    println!("删除: {}", diff.deleted_count());
    println!("修改: {}", diff.modified_count());
    println!("未变更: {}", diff.unchanged_count());
    println!("----------------------------------------");
    
    println!("\n新增条目:");
    for entry in diff.entries.iter().filter(|e| e.diff_type == DiffType::Added) {
        println!("  + {} ({})", entry.path, 
            if entry.entry_type == EntryType::File { "文件" } else { "目录" });
    }
    
    println!("\n删除条目:");
    for entry in diff.entries.iter().filter(|e| e.diff_type == DiffType::Deleted) {
        println!("  - {} ({})", entry.path, 
            if entry.entry_type == EntryType::File { "文件" } else { "目录" });
    }
    
    println!("\n修改条目:");
    for entry in diff.entries.iter().filter(|e| e.diff_type == DiffType::Modified) {
        println!("  * {} ({})", entry.path, 
            if entry.entry_type == EntryType::File { "文件" } else { "目录" });
        
        if entry.entry_type == EntryType::File {
            if let (Some(local), Some(remote)) = (&entry.local_state, &entry.remote_state) {
                println!("    本地: 大小={:?}, 修改时间={:?}", 
                         local.size, local.modified);
                println!("    远程: 大小={:?}, 修改时间={:?}", 
                         remote.size, remote.modified);
            }
        }
    }
