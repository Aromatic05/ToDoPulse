...
use crate::function::sync::model::{
    FileSystemState, DiffResult, SyncSession, SyncOperation, SyncOperationType,
    SyncOperationStatus, SyncSessionStatus, ConflictStrategy, EntryState
};
use crate::function::sync::state::{collect_local_state, StateCollectionConfig, save_state, load_state};
use crate::function::sync::diff::{compare_states, DiffConfig};
use crate::function::sync::webdav::{
    create_client, ensure_remote_dir_exists,
...
/// 收集本地和远程状态
pub async fn collect_states(session: &mut SyncSession) -> Result<(FileSystemState, FileSystemState)> {
    ...
    let webdav_remote_state = crate::function::sync::webdav::collect_remote_state(&client, &session.remote_dir).await?;
    ...
    for (path, webdav_entry) in &webdav_remote_state.entries {
        if webdav_entry.is_file() {
            ...
            let entry = EntryState::new_directory(
                path.clone(),
                webdav_entry.modified
            );
            remote_state.add_entry(entry);
                ...
                crate::function::sync::model::DiffType::Modified => {
                    ...
                    match conflict_strategy {
                        ...
                        ConflictStrategy::KeepBoth => {
                            ...
                            let stem = Path::new(&entry.path).file_stem()
                                .and_then(|s| s.to_str())
                                .unwrap_or("");
...
pub async fn execute_sync_operations(session: &mut SyncSession) -> Result<()> {
    ...
    for operation in &mut session.operations {
        ...
        match operation.operation_type {
            SyncOperationType::Upload => {
                ...
                let local_path = session.local_dir.join(operation.path.trim_start_matches('/'));
    ...
    // 更新会话状态
    let failed_count = session.operations.iter()
...
/// 保存和加载同步状态
pub async fn save_sync_state(local_state: &FileSystemState, remote_state: &FileSystemState) -> Result<()> {
