...
mod tests {
    ...
    async fn test_delete() {
        ...
        Repository::<List>::add(storage, &list).unwrap();
        assert!(Repository::<List>::get_by_name(storage, &list_id)
            .unwrap()
            .is_some());
    ...
    async fn test_ensure_table_exists() {
        ...
        println!(
            "test_ensure_table_exists completed in: {:?}",
            start.elapsed()
        );
    }
    ...
    async fn test_performance_bulk_operations() {
        ...
        println!(
            "Bulk filter (found {} items) completed in: {:?}",
            filtered.len(),
            filter_time
        );

        println!(
            "test_performance_bulk_operations total time: {:?}",
            start.elapsed()
        );
    }
}
