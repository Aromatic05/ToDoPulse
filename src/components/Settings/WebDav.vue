    ...
    <v-expansion-panel>
        ...
        <v-expansion-panel-text>
            <v-form @submit.prevent="testConnection">
                <v-row>
                    <v-col cols="12">
                        <v-text-field v-model="host" label="WebDAV 服务器地址" placeholder="https://example.com/dav/"
                            variant="outlined" density="compact" class="mb-3" required></v-text-field>
                    </v-col>
                    <v-col cols="12" md="6">
                    ...
                    <v-col cols="12">
                        <div class="d-flex gap-4">
                            <v-btn color="primary" @click="testConnection" :loading="isTesting" class="flex-grow-1">
                                测试连接
                            </v-btn>
                            <v-btn color="success" @click="sync" :loading="isSyncing" :disabled="!connectionTested"
                                class="flex-grow-1">
                                开始同步
                            </v-btn>
                        </div>
            ...
            <v-alert v-if="result" :color="result.success ? 'success' : 'error'"
                :title="result.success ? '操作成功' : '操作失败'" :text="result.message" class="mt-3" variant="tonal" closable
                @click:close="clearResult"></v-alert>
        </v-expansion-panel-text>
    </v-expansion-panel>
</template>
