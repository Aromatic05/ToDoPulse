<template>
    <div class="calendar-container">
        ...
        <!-- 添加日历事件上下文菜单 -->
        <CalendarContextMenu v-model:show="showContextMenu" :activator-element="contextMenuActivator"
            :target-event="contextMenuEvent" @toggle-completion="handleToggleCompletion" @delete="handleEventDelete" />
    </div>
</template>
    ...
    setup() {
        ...
        const loadCalendarEvents = async () => {
            ...
            if (fullCalendar.value) {
                ...
                setTimeout(() => {
                    if (fullCalendar.value) {
                        const eventElements = fullCalendar.value.$el.querySelectorAll('.fc-event');
                        eventElements.forEach((el: HTMLElement) => {
                            const eventId = el.getAttribute('data-event-id') ||
                                (el as any).fcSeg?.eventRange?.def?.publicId;
                            if (eventId) {
                                const event = fullCalendar.value!.getApi().getEventById(eventId);
                                if (event) {
        ...
        const handleEventClick = (clickInfo: EventClickArg) => {
            ...
            eventEl.addEventListener('touchstart', (e) => handleEventTouchStart(clickInfo, e as TouchEvent));
        ...
        const handleEventTouchStart = (info: EventClickArg, e: TouchEvent) => {
            ...
            longPressTimer.value = window.setTimeout(() => {
                ...
                const touchTarget = document.elementFromPoint(touch.clientX, touch.clientY) as HTMLElement;
